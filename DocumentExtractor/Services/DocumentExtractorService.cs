...
public class DocumentExtractorService
{
    ...
    public async Task<ExtractDocumentResponse> ExtractDataFromDocument(ExtractDocumentModel documentModel)
    {
       ...
       // response.Paragraphs = result?.Paragraphs?.ToList();
        response.tableCells = await ExtractCells(result);
    ...
    public async Task<List<TableCell>> ExtractCells(AnalyzeResult result)
    {
        try
        {
List<string> headers = new List<string>();
        ...
        for (int i = 0; i < result.Tables.Count; i++)
        {
            ...
            List<DocumentTableCell> tableCells = new List<DocumentTableCell>();


            foreach (DocumentTableCell tableCell in table.Cells)
            {

                if (tableCell.Kind == "columnHeader")
                {
                    tableCells.Add(tableCell);
                    headers.Add(tableCell.Content);
                ...
                else
                {
                    ...
                    {
                        var header = tableCells.FirstOrDefault(tc => tc.ColumnIndex == tableCell.ColumnIndex);
                        keyValues.Add(header?.Content ?? "", tableCell?.Content);
                        cell.Cell.Add(keyValues);
                    }
    ...
    public async Task<DocumentLayoutResponse> ExtractDocumentLayout(IFormFile file)
    {
        ...
        //         Console.WriteLine($"    Cell ({tableCell.RowIndex}, {tableCell.ColumnIndex}) has kind '{tableCell.Kind}' and content: '{tableCell.Content}'.");
        //     }
        ...
        // layoutResponse.tableCells = await ExtractCells(result);
